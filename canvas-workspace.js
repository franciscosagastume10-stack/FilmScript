(function () {
  const STYLE = `
    ...
    *{box-sizing:border-box}button,input,textarea,select{font:inherit}button{color:inherit}.cv-root{min-height:560px;animation:cv-in .2s cubic-bezier(.2,.8,.2,1) both}.cv-loading{display:grid;place-items:center;min-height:520px;color:var(--cv-muted);font-size:13px}.cv-loading span{width:22px;height:22px;border:2px solid var(--cv-hair);border-top-color:var(--cv-accent);border-radius:50%;animation:cv-spin .7s linear infinite;margin-bottom:12px}.cv-load-stack{display:grid;justify-items:center}.cv-top{display:flex;align-items:center;gap:18px;padding:0 0 24px;border-bottom:1px solid var(--cv-hair)}.cv-title{margin-right:auto}.cv-eyebrow{font-size:9.5px;line-height:1;text-transform:uppercase;letter-spacing:1.5px;font-weight:750;color:var(--cv-accent)}.cv-title h1{margin:8px 0 0;font-size:30px;line-height:1;letter-spacing:-.8px}.cv-tabs{display:flex;gap:3px;padding:3px;background:color-mix(in srgb,var(--cv-soft) 72%,transparent);border-radius:11px}.cv-tab{border:0;background:transparent;border-radius:8px;padding:8px 12px;font-size:11.5px;font-weight:600;cursor:pointer;transition:background .15s ease,color .15s ease,transform .12s ease}.cv-tab:hover{background:var(--cv-surface)}.cv-tab[aria-current=true]{color:var(--cv-accent);background:var(--cv-surface);box-shadow:0 1px 5px rgba(30,28,24,.07)}.cv-icon-btn{width:34px;height:34px;display:grid;place-items:center;border:1px solid var(--cv-hair);background:var(--cv-surface);border-radius:9px 10px 8px 9px;cursor:pointer;transition:transform .13s ease,border-color .15s ease,background .15s ease}.cv-icon-btn:hover{border-color:var(--cv-muted);transform:translateY(-1px)}.cv-icon-btn svg{width:16px;height:16px}.cv-heading{display:flex;align-items:flex-end;justify-content:space-between;gap:24px;margin:32px 0 18px}.cv-heading h2{margin:0;font-size:21px;letter-spacing:-.35px}.cv-heading p{max-width:580px;margin:7px 0 0;color:var(--cv-muted);font-size:12.5px;line-height:1.5}.cv-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.cv-btn{min-height:36px;padding:0 13px;border:1px solid var(--cv-hair);border-radius:9px 10px 8px 9px;background:var(--cv-surface);color:var(--cv-ink);font-size:11.5px;font-weight:650;cursor:pointer;transition:transform .12s ease,border-color .15s ease,background .15s ease,box-shadow .15s ease}.cv-btn:hover{border-color:var(--cv-muted);transform:translateY(-1px);box-shadow:0 5px 14px rgba(30,28,24,.07)}.cv-btn:active{transform:scale(.98)}.cv-btn.primary{background:var(--cv-ink);border-color:var(--cv-ink);color:var(--cv-surface)}.cv-btn.accent{background:var(--cv-accent);border-color:var(--cv-accent);color:#181716}.cv-btn.danger{color:#B24C47;border-color:color-mix(in srgb,#B24C47 45%,var(--cv-hair))}.cv-btn:disabled{opacity:.45;cursor:not-allowed;transform:none}.cv-card{position:relative;background:var(--cv-surface);border:1px solid color-mix(in srgb,var(--cv-ink) 52%,var(--cv-hair));border-radius:16px 14px 17px 13px}.cv-card:after{content:'';position:absolute;inset:4px -3px -4px 3px;z-index:-1;border:1px solid color-mix(in srgb,var(--cv-ink) 16%,transparent);border-radius:14px 17px 13px 16px;pointer-events:none}.cv-tool-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}.cv-tool{min-height:210px;padding:24px;text-align:left;cursor:pointer;overflow:hidden;transition:transform .18s cubic-bezier(.2,.8,.2,1),box-shadow .18s ease,border-color .18s ease}.cv-tool:hover{transform:translateY(-3px);border-color:var(--cv-accent);box-shadow:0 16px 34px rgba(34,31,25,.09)}.cv-tool-no{font-size:9px;letter-spacing:1.3px;color:var(--cv-muted);font-weight:750}.cv-tool-icon{width:45px;height:45px;display:grid;place-items:center;margin-top:26px;border-radius:13px 11px 14px 10px;background:var(--cv-accent-soft);color:var(--cv-accent)}.cv-tool-icon svg{width:23px;height:23px}.cv-tool h3{font-size:19px;margin:18px 0 7px}.cv-tool p{margin:0;color:var(--cv-muted);font-size:12px;line-height:1.5}.cv-tool-arrow{position:absolute;right:22px;bottom:20px;color:var(--cv-muted);transition:transform .18s ease,color .18s ease}.cv-tool:hover .cv-tool-arrow{transform:translateX(3px);color:var(--cv-accent)}
    .cv-role-line{display:flex;align-items:center;gap:9px;margin-top:20px;color:var(--cv-muted);font-size:11px}.cv-role-dot{width:7px;height:7px;border-radius:50%;background:var(--cv-accent);box-shadow:0 0 0 5px var(--cv-accent-soft)}
  ...
  const roleLabels = {
    director: 'Director', producer: 'Producer', director_of_photography: 'Director of Photography',
    production_designer: 'Production Designer', sound_recordist: 'Sound Recordist',
  };
  ...
  class FilmScriptCanvasWorkspace extends HTMLElement {
    ...
    constructor() {
      ...
      this.state = {
        ...
        itemModal: false, editingItemId: '', roleModal: false, boardModal: false,
    ...
    async load() {
      ...
      try {
        ...
        this.state.error = '';
        this.state.view = result.workspace?.settings?.lastTool || 'home';
        if (!result.workspace?.role) this.state.roleModal = true;
      } catch (error) {
        this.state.loading = false;
    ...
    _renderTop() {
      return `<header class="cv-top"><div class="cv-title"><div class="cv-eyebrow">Visual production</div><h1>Canvas</h1></div><nav class="cv-tabs" aria-label="Canvas tools"><button class="cv-tab" data-action="view-home" aria-current="${this.state.view === 'home'}">Home</button><button class="cv-tab" data-action="view-vault" aria-current="${this.state.view === 'vault'}">Vault</button><button class="cv-tab" data-action="view-boards" aria-current="${this.state.view === 'boards'}">Boards</button></nav><button class="cv-icon-btn" data-action="role-settings" aria-label="Canvas settings" title="Canvas settings">${icon('settings')}</button></header>`;
    }

    _toolOrder() {
      const role = this.state.workspace?.role;
      const orders = {
        production_designer: ['vault', 'boards', 'shotlist'], director: ['boards', 'shotlist', 'vault'],
        producer: ['vault', 'shotlist', 'boards'], director_of_photography: ['shotlist', 'boards', 'vault'],
        sound_recordist: ['boards', 'vault', 'shotlist'],
      };
      return orders[role] || ['shotlist', 'vault', 'boards'];
    }
    ...
    _renderHome() {
      ...
      const cards = this._toolOrder().map((id, index) => {
      ...
      return `<section><div class="cv-heading"><div><h2>Your visual workspace</h2><p>Develop the visual language of ${esc(this.projectTitle)}, select production assets, and prepare client-ready documents without leaving the project.</p></div></div><div class="cv-tool-grid">${cards}</div><div class="cv-role-line"><span class="cv-role-dot"></span><span>Ordered for <strong>${esc(roleLabels[this.state.workspace.role] || 'your production role')}</strong>. Change this anytime in Canvas settings.</span></div></section>`;
    }
    ...
    _renderVaultCard(item) {
      ...
      return `<article class="cv-card cv-item ${selected ? 'selected' : ''}" data-item-id="${item.id}"><input class="cv-item-check" type="checkbox" data-select-item="${item.id}" ${selected ? 'checked' : ''} aria-label="Select ${esc(item.name)}"><button class="cv-item-menu" data-action="item-menu" data-id="${item.id}" aria-label="Item actions">•••</button><div class="cv-item-image" data-action="edit-item" data-id="${item.id}">${image}</div><div class="cv-item-body"><div><div class="cv-item-title">${esc(item.name)}</div><div class="cv-item-meta"><span class="cv-status ${esc(item.availability)}">${esc(item.quantityAvailable)} available</span><span class="cv-item-price">${money(item.dailyPrice)}/day</span></div></div></div></article>`;
    }
    ...
    _renderBoards() {
      ...
      if (!boards.length) return `<section><div class="cv-heading"><div><h2>Boards</h2><p>Open visual workspaces for art direction, photography, video, and mixed references.</p></div></div><div class="cv-card cv-empty"><div>${icon('boards').replace('<svg','<svg class="cv-empty-art"')}<h3>Start building your visual world</h3><p>Create an Art, Photo, or Video Board to organize references, concepts, and production ideas.</p><button class="cv-btn accent" data-action="create-board">Create Board</button></div></div></section>`;
      return `<section><div class="cv-heading"><div><h2>Boards</h2><p>${boards.length} visual workspace${boards.length === 1 ? '' : 's'} with autosave and free placement.</p></div><button class="cv-btn accent" data-action="create-board">+ Create Board</button></div><div class="cv-board-grid">${boards.map((board) => `<article class="cv-card cv-board-card" data-action="open-board" data-id="${board.id}"><button class="cv-item-menu cv-board-menu" data-action="board-menu" data-id="${board.id}" aria-label="Board actions">•••</button><div class="cv-board-preview"></div><h3>${esc(board.title)}</h3><p>${esc(typeLabels[board.type] || 'Blank')} Board · ${board.elements.length} element${board.elements.length === 1 ? '' : 's'}</p></article>`).join('')}</div></section>`;
    }

    _renderOverlays() {
      const overlays = [];
      if (this.state.roleModal) overlays.push(this._renderRoleModal());
    ...
    _renderRoleModal() {
      const first = !this.state.workspace?.role;
      return `<div class="cv-modal-backdrop" data-action="${first ? 'none' : 'close-role'}"><section class="cv-modal" role="dialog" aria-modal="true" aria-label="Canvas production role" data-stop><header class="cv-modal-head"><div><div class="cv-eyebrow">${first ? 'Welcome to Canvas' : 'Canvas settings'}</div><h3>What is your role in this production?</h3><p>This only changes the order of the tools already inside Canvas.</p></div>${first ? '' : `<button class="cv-icon-btn cv-close" data-action="close-role" aria-label="Close">×</button>`}</header><div class="cv-modal-body"><div class="cv-role-grid">${Object.entries(roleLabels).map(([role,label]) => `<button class="cv-role ${this.state.workspace?.role === role ? 'selected' : ''}" data-action="choose-role" data-role="${role}"><span class="cv-role-icon">${icon(role === 'director_of_photography' ? 'shot' : role === 'production_designer' ? 'vault' : 'boards')}</span><span><strong>${label}</strong><small>${role === 'production_designer' ? 'Vault and Boards first' : role === 'director_of_photography' ? 'Shot List and visual references first' : 'Tools ordered for this perspective'}</small></span></button>`).join('')}</div></div></section></div>`;
    }
    ...
    _renderBoardEditor() {
      ...
      const empty = !board.elements.length ? `<div class="cv-board-empty"><h3>${esc(typeLabels[board.type])} Board</h3><p>Place only what helps communicate the visual direction.</p><span>Upload references</span><span>Add from Vault</span><span>Add text or notes</span></div>` : '';
      const toolbar = selected.size ? `<div class="cv-element-toolbar" style="left:18px;top:70px"><button data-action="board-duplicate-selection">Duplicate</button><button data-action="board-group-selection">Group</button><button data-action="board-align-left">Align left</button><button data-action="board-delete-selection" style="color:#B24C47">Delete</button></div>` : '';
      return `<div class="cv-board-shell"><header class="cv-board-top"><button class="cv-btn cv-board-back" data-action="back-boards">${icon('back')} Boards</button><input class="cv-board-title" data-field="board-title" value="${esc(board.title)}" aria-label="Board title"><span class="cv-board-save">${esc(this.state.autosave)}</span><button class="cv-icon-btn" data-action="board-undo" title="Undo" ${this._history.length ? '' : 'disabled'}>${icon('undo')}</button><button class="cv-icon-btn" data-action="board-redo" title="Redo" ${this._future.length ? '' : 'disabled'}>${icon('redo')}</button><button class="cv-btn" data-action="board-add-image">${icon('image')} Image</button><button class="cv-btn" data-action="board-add-text">Text</button><button class="cv-btn" data-action="board-add-note">Note</button><button class="cv-btn" data-action="board-add-vault">Vault</button><button class="cv-btn" data-action="board-zoom-out">−</button><span style="font-size:10px;color:var(--cv-muted);min-width:38px;text-align:center">${Math.round(board.viewport.zoom * 100)}%</span><button class="cv-btn" data-action="board-zoom-in">+</button><button class="cv-icon-btn ${board.settings.snapToGrid ? 'active' : ''}" data-action="board-snap" title="Snap to grid">${icon('grid')}</button><input hidden type="file" accept="image/png,image/jpeg,image/webp" data-file="board-image"></header><div class="cv-board-viewport" data-board-viewport tabindex="0"><div class="cv-board-layer" data-board-layer>${elements}</div>${empty}</div>${toolbar}</div>`;
    }
    ...
    _onClick(event) {
      ...
      if (action === 'role-settings') { this.state.roleModal = true; return this.render(); }
      if (action === 'close-role') { this.state.roleModal = false; return this.render(); }
      if (action === 'choose-role') return this.chooseRole(trigger.dataset.role);
      ...
      if (action === 'board-add-image' || action === 'board-upload') { this.state.boardContext = null; this.render(); return this.shadowRoot.querySelector('[data-file="board-image"]')?.click(); }
    ...
    _onChange(event) {
      ...
      if (field === 'vault-category') this.state.category = event.target.value;
        ...
        return;
      } else if (event.target.dataset.file === 'vault-import') return this.importVault(event.target.files?.[0]);
      else if (event.target.dataset.file === 'board-image') return this.uploadBoardImage(event.target.files?.[0]);
    ...
    async chooseRole(role) {
      try {
        const result = await window.filmscriptCanvas.update(this.scriptId, { role });
        this.state.workspace = result.workspace;
        this.state.roleModal = false;
        this.render();
    ...
    async compressImage(file) {
      ...
      } catch { return { file, width: 0, height: 0 }; }
    }
    ...
    addBoardElement(type,extra={}){const board=this.activeBoard();if(!board)return;this.pushHistory();const point=this._boardPoint();if(!this.state.boardContext&&board.elements.length){const slot=board.elements.length%6;const cycle=Math.floor(board.elements.length/6);const offsets=[[0,0],[360,0],[-360,0],[0,230],[360,230],[-360,230]][slot];point.x+=offsets[0]+cycle*28;point.y+=offsets[1]+cycle*28;}const element={id:uid('bel'),type,positionX:point.x,positionY:point.y,width:type==='text'?320:220,height:type==='text'?90:150,rotation:0,zIndex:Math.max(0,...board.elements.map((entry)=>entry.zIndex||0))+1,content:type==='text'?'Title':type==='note'?'Add a note…':'',metadata:{},status:'',locked:false,hidden:false,groupId:'',sceneId:'',vaultItemId:'',assetId:'',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),...extra};if(board.settings.snapToGrid){const g=board.settings.gridSize;element.positionX=Math.round(element.positionX/g)*g;element.positionY=Math.round(element.positionY/g)*g;}board.elements.push(element);this.state.selected=new Set([element.id]);this.state.boardContext=null;this.queueBoardSave();this.render();}
    async uploadBoardImage(file){if(!file)return;this.toast('Optimizing and uploading image…');try{const [asset]=await this.uploadFiles([file]);this.addBoardElement('image',{assetId:asset.id,width:300,height:220,content:file.name});this.toast('Image added to Board');}catch(error){this.toast(error.message);}}
