...
## Resumen
FilmScript es un editor de guiones con un asistente de IA llamado Lumiere. Ofrece Free para producción manual con créditos únicos de bienvenida, FilmScript Creator ($24.99 al mes) para todas las funciones de texto con IA, y FilmScript Full ($39.99 al mes) con generación de imágenes. Full incluye 1,000 créditos de imagen por ciclo y cada imagen cuesta 3 créditos. Los identificadores históricos `basic` y `lumiere` se conservan únicamente para reconocer suscripciones existentes y se resuelven como Creator. Este documento describe el backend que verifica la transacción y activa cada plan solo cuando Recurrente confirma el pago.
...
- El botón "Pay with Recurrente" abre el checkout real del plan seleccionado.
...
- El prototipo antiguo guardaba el plan en `localStorage`; la implementación actual usa el backend y mantiene `basic` y `lumiere` como identificadores internos.
...
### 3. `POST /api/checkout`
Recibe `{ plan: "creator" | "full" }` del frontend (usuario autenticado). `creator` desbloquea IA de texto y `full` añade generación de imágenes con créditos mensuales.
...
Recomendación: crear FilmScript Creator y FilmScript Full como productos de suscripción en el panel de Recurrente y referenciarlos por id, en lugar de mandar montos sueltos.
...
### 5. `GET /api/me`
Devuelve `{ email, tier: "free" | "creator" | "full", credits, entitlements }` leyendo la base de datos. El frontend lo consulta al cargar y después de volver del checkout. Los identificadores heredados se convierten a `creator` antes de exponerse al cliente.

### 6. `POST /api/subscription/cancel`
Llama a la API de Recurrente para cancelar la suscripción del usuario y actualiza la base de datos. Conecta con el botón de cancelación del plan activo en el panel de cuenta.

## Esquema mínimo de base de datos
- `users`: id, email, nombre, fecha de creación.
- `subscriptions`: id, user_id, plan (`creator` o `full`; los valores heredados se resuelven como Creator), status (`pending`, `active`, `canceled`, `past_due`), recurrente_checkout_id, recurrente_subscription_id, updated_at.
...
- Panel de cuenta (avatar): el botón de cancelación del plan activo llama al endpoint real.
