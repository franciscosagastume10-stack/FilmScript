...
class FilmScriptBudget extends HTMLElement {
  ...
  _onClick = (event) => {
    ...
    if (target.dataset.view) {
      ...
      if (nextView === 'breakdown' && target.dataset.phase) {
        this.phaseFilter = target.dataset.phase;
        this.openAccounts = new Set((this.budget?.accounts || [])
          .filter((account) => account.phaseId === target.dataset.phase)
          .map((account) => account.code));
      }
      this.view = nextView;
      this.scheduleItemId = '';
  ...
  renderQuick(computed) {
    ...
    const phasePastels = { above_line: '#D9B77D', production: '#A8C39A', postproduction: '#A9C3DD', other: '#C4A3C8' };
    const donutRadius = 67;
    const donutCircumference = 2 * Math.PI * donutRadius;
    let donutCursor = 0;
    const donutSegments = phasesWithValue.length ? phasesWithValue.map((phase) => {
      const phaseItems = computed.accounts
        .filter((account) => account.phaseId === phase.id)
        .flatMap((account) => account.items)
        .filter((item) => item.total > 0.005 || item.spent > 0.005)
        .sort((a, b) => (b.total + b.spent) - (a.total + a.spent));
      const topItems = phaseItems.slice(0, 2);
      const topLine = topItems[0];
      const topLineCopy = topItems.length
        ? topItems.map((item) => `${escapeHtml(item.name)} · ${this.formatMoney(item.total)}`).join('<br>')
        : 'No line items yet';
      const phasePercent = this.formatPercent(phase.share);
      const phaseAmount = this.formatMoney(phase.total);
      const pastel = phasePastels[phase.id] || '#C5BBA3';
      const start = donutCursor;
      const segmentLength = phase.share * donutCircumference;
      const midAngle = -Math.PI / 2 + (start + phase.share / 2) * Math.PI * 2;
      const labelRadius = 88;
      const labelX = 100 + Math.cos(midAngle) * labelRadius;
      const labelY = 100 + Math.sin(midAngle) * labelRadius;
      donutCursor += phase.share;
      return `<g class="donut-segment" style="--segment-color:${pastel}" data-view="breakdown" data-phase="${phase.id}" tabindex="0" role="button" aria-label="Open ${escapeHtml(phase.name)} in Budget Breakdown" title="${escapeHtml(`${phase.name}: ${phaseAmount} · ${phasePercent}`)}">
        <title>${escapeHtml(`${phase.name}: ${phaseAmount} · ${phasePercent}. Click to open Breakdown.`)}</title>
        <circle class="donut-segment-ring" cx="100" cy="100" r="${donutRadius}" pathLength="${donutCircumference}" stroke="${pastel}" stroke-dasharray="${segmentLength} ${donutCircumference}" stroke-dashoffset="-${start * donutCircumference}" transform="rotate(-90 100 100)"></circle>
        <g class="donut-segment-label" transform="translate(${labelX.toFixed(1)} ${labelY.toFixed(1)})"><rect x="-19" y="-10" width="38" height="20" rx="10" fill="${pastel}"></rect><text x="0" y="3.5" text-anchor="middle">${phasePercent}</text></g>
      </g>`;
    }).join('') : '';
    const donutLegend = phasesWithValue.length ? phasesWithValue.map((phase) => {
      const pastel = phasePastels[phase.id] || '#C5BBA3';
      return `<button type="button" class="donut-legend-row" style="--segment-color:${pastel}" data-view="breakdown" data-phase="${phase.id}" aria-label="Open ${escapeHtml(phase.name)} in Budget Breakdown"><i style="background:${pastel}"></i><span>${escapeHtml(phase.name)}</span><strong>${this.formatPercent(phase.share)}</strong><small>${this.formatMoney(phase.total)}</small></button>`;
    }).join('') : '<div class="chart-empty-copy"><strong>No allocation yet</strong><span>Add the first cost in Breakdown.</span></div>';
    const allocationGraphic = phasesWithValue.length ? `<div class="donut-graphic"><svg class="donut-svg" viewBox="0 0 200 200" role="img" aria-label="Budget allocation by production phase"><circle class="donut-track" cx="100" cy="100" r="${donutRadius}" pathLength="${donutCircumference}"></circle>${donutSegments}<circle class="donut-hole" cx="100" cy="100" r="48"></circle><text class="donut-total" x="100" y="98" text-anchor="middle">${this.moneyValue(computed.total)}</text><text class="donut-total-caption" x="100" y="114" text-anchor="middle">TOTAL</text></svg></div>` : '<div class="donut-graphic donut-empty"><span>No allocation yet</span></div>';
    const allocationRows = `<div class="donut-wrap"><div class="donut-graphic-wrap">${allocationGraphic}</div><div class="donut-legend" role="list" aria-label="Budget allocation details">${donutLegend}</div></div>`;
    ...
    return `<section class="view quick-view" role="tabpanel" tabindex="0" aria-labelledby="budget-quick-title">
      ...
      <div class="visual-grid">
        <article class="panel allocation"><div class="panel-head"><div><span>Allocation</span><h3>Where the budget goes</h3></div><small>Percent of approved total · click a segment</small></div>${allocationRows}</article>
  ...
  styles() {
    return `<style>
      ...
      *{box-sizing:border-box}button,input,select{font:inherit}button{color:inherit}.fs-budget{padding:24px 0 72px}.budget-nav{position:sticky;top:-38px;z-index:20;display:flex;align-items:center;justify-content:space-between;gap:18px;margin:0 0 28px;padding:12px 0;background:var(--bg,#F5F0E8);border-bottom:1px solid var(--hair,#E7E4DA)}.tabs{display:flex;gap:4px;overflow:auto;scrollbar-width:none}.tabs button{min-height:38px;padding:0 13px;border:1px solid transparent;border-radius:9px 10px 8px 9px;background:transparent;color:var(--muted,#888780);white-space:nowrap;cursor:pointer}.tabs button[aria-pressed="true"]{border-color:var(--hair,#E7E4DA);background:var(--surface,#FFFEF9);color:var(--ink,#2C2C2A);box-shadow:1px 2px 0 var(--hair,#E7E4DA)}.tabs button:focus-visible,.export:focus-visible,button:focus-visible,input:focus-visible,select:focus-visible{outline:2px solid var(--accent,#BA7517);outline-offset:2px}.budget-actions{display:flex;align-items:center;gap:10px}.budget-actions span{font-size:10.5px;color:var(--muted,#888780);white-space:nowrap}.export,.primary{min-height:40px;padding:0 16px;border:0;border-radius:9px 10px 8px 9px;background:var(--accent,#BA7517);color:#181816;font-weight:750;cursor:pointer}.export{background:var(--ink,#2C2C2A);color:var(--surface,#FFFEF9)}.secondary{min-height:38px;padding:0 14px;border:1px solid var(--hair,#E7E4DA);border-radius:9px;background:var(--surface,#FFFEF9);font-weight:700;cursor:pointer}.view{animation:budgetRise .22s ease both}@keyframes budgetRise{from{opacity:0;transform:translateY(7px)}to{opacity:1;transform:none}}.section-head{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin-bottom:22px}.eyebrow,.panel-head span,.finance-note>span,.formula-card>span,.insight>span{display:block;font-size:10px;font-weight:800;letter-spacing:1.45px;text-transform:uppercase;color:var(--accent,#BA7517)}h2{margin:6px 0 0;font-size:27px;line-height:1.1;letter-spacing:-.7px}h3{margin:4px 0 0;font-size:16px;letter-spacing:-.2px}.section-head p,.table-title p,.panel-head small,.modal-head p{margin:7px 0 0;color:var(--muted,#888780);font-size:12px;line-height:1.45}.updated{font-size:11px;color:var(--muted,#888780)}.kpi-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.kpi{position:relative;min-height:126px;padding:19px 19px 17px;background:var(--surface,#FFFEF9);border:1px solid var(--hair,#E7E4DA);border-radius:15px 13px 16px 12px;box-shadow:2px 3px 0 var(--hair,#E7E4DA);overflow:hidden}.kpi:before{content:"";position:absolute;left:0;top:0;bottom:0;width:3px;background:var(--accent,#BA7517)}.kpi.remaining:before,.kpi.funded:before{background:#5B7A4A}.kpi.spent:before{background:#4A6B8A}.kpi.warning:before{background:#BA7517}.kpi.danger:before{background:#C74440}.kpi span{display:block;font-size:10.5px;font-weight:700;color:var(--muted,#888780)}.kpi strong{display:block;margin-top:17px;font-size:21px;letter-spacing:-.55px;font-variant-numeric:tabular-nums}.kpi small{display:block;margin-top:8px;color:var(--muted,#888780);font-size:10.5px}.compact{margin-bottom:20px}.compact .kpi{min-height:110px}.visual-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:14px}.panel,.table-card{background:var(--surface,#FFFEF9);border:1px solid var(--hair,#E7E4DA);border-radius:17px 14px 18px 13px;box-shadow:2px 3px 0 var(--hair,#E7E4DA)}.panel{padding:20px}.panel-head,.table-title{display:flex;align-items:flex-start;justify-content:space-between;gap:18px}.donut-wrap{display:grid;grid-template-columns:168px 1fr;align-items:center;gap:24px;margin-top:22px}.donut{width:164px;height:164px;border-radius:50%;display:grid;place-items:center}.donut:before{content:"";position:absolute}.donut>span{width:104px;height:104px;border-radius:50%;display:grid;place-content:center;text-align:center;background:var(--surface,#FFFEF9);box-shadow:0 0 0 1px var(--hair,#E7E4DA)}.donut strong{font-size:14px}.donut small{margin-top:4px;font-size:9.5px;color:var(--muted,#888780)}.legend{display:grid;gap:11px}.legend div{display:grid;grid-template-columns:9px 1fr auto;align-items:center;gap:8px;font-size:11px}.legend i{width:9px;height:9px;border-radius:3px}.legend strong{font-size:10.5px}.phase-bars{display:grid;gap:18px;margin-top:25px}.phase-bar>div:first-child{display:flex;justify-content:space-between;gap:12px;margin-bottom:7px}.phase-bar strong{font-size:11px}.phase-bar span{font-size:10px;color:var(--muted,#888780)}.bar{height:8px;border-radius:99px;background:var(--soft,#EFEBE1);overflow:hidden}.bar i{display:block;height:100%;min-width:0;border-radius:99px}.insight{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:18px;margin-top:14px;padding:16px 18px;background:var(--accent-soft,rgba(186,117,23,.11));border:1px solid var(--accent,#BA7517);border-radius:12px 14px 11px 13px}.insight p{margin:0;font-size:11.5px;line-height:1.45}.insight button,.finance-note button{border:0;background:transparent;color:var(--accent,#BA7517);font-weight:750;font-size:11px;cursor:pointer}.phase-card-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:18px}.phase-card{padding:17px 18px;background:var(--surface,#FFFEF9);border:1px solid var(--hair,#E7E4DA);border-top:3px solid var(--phase);border-radius:12px}.phase-card span,.phase-card small{display:block;font-size:10.5px;color:var(--muted,#888780)}.phase-card strong{display:block;margin:10px 0 7px;font-size:17px}.table-card{overflow:hidden}.table-title{align-items:center;padding:18px 19px;border-bottom:1px solid var(--hair,#E7E4DA)}.table-title h3{margin:0}.table-scroll{overflow:auto}.table-card table{width:100%;border-collapse:collapse;min-width:820px}th{padding:11px 12px;border-bottom:1px solid var(--hair,#E7E4DA);color:var(--muted,#888780);font-size:9px;letter-spacing:1.05px;text-transform:uppercase;text-align:left;white-space:nowrap}td{padding:11px 12px;border-bottom:1px solid var(--hair,#E7E4DA);font-size:10.8px;white-space:nowrap;font-variant-numeric:tabular-nums}td:not(:nth-child(2)){text-align:right}tbody tr:hover{background:var(--soft,#EFEBE1)}tfoot td{background:var(--chrome,#232322);color:var(--surface,#FFFEF9);font-weight:750;border:0}.account-code{display:inline-grid;place-items:center;min-width:43px;height:23px;padding:0 6px;border-radius:7px;background:var(--soft,#EFEBE1);font-weight:750}.negative{color:#C74440!important;font-weight:750}.filters{display:grid;grid-template-columns:minmax(240px,1fr) 180px minmax(260px,auto);align-items:end;gap:12px;margin-bottom:16px}.filters label,.form-grid label,.schedule-grid label{display:grid;gap:6px}.filters label span,.form-grid label>span,.schedule-grid label>span{font-size:9.5px;font-weight:700;color:var(--muted,#888780)}input,select{min-height:36px;border:1px solid var(--hair,#E7E4DA);border-radius:8px;background:var(--bg,#F5F0E8);color:var(--ink,#2C2C2A);padding:0 9px;outline:none}input:focus,select:focus{border-color:var(--accent,#BA7517)}.formula-note{display:flex;align-items:center;justify-content:space-between;gap:12px;min-height:58px;padding:10px 13px;border:1px solid var(--hair,#E7E4DA);border-radius:10px;background:var(--surface,#FFFEF9)}.formula-note strong{font-size:10px}.formula-note span{font-size:10px;color:var(--muted,#888780)}.account-stack{display:grid;gap:10px}.account{background:var(--surface,#FFFEF9);border:1px solid var(--hair,#E7E4DA);border-radius:13px 15px 12px 14px;overflow:hidden}.account summary{display:flex;justify-content:space-between;align-items:center;gap:20px;padding:15px 17px;cursor:pointer;list-style:none}.account summary::-webkit-details-marker{display:none}.account summary>div{display:flex;align-items:center;gap:11px}.account summary>div:first-child>span{display:grid;place-items:center;min-width:45px;height:25px;border-radius:7px;background:var(--soft,#EFEBE1);font-size:10px;font-weight:800}.account summary strong{font-size:12px}.account summary small,.account summary>div:last-child>span{font-size:10px;color:var(--muted,#888780)}.breakdown-table{min-width:1510px;width:100%;border-collapse:collapse}.breakdown-table th,.breakdown-table td{padding:7px 6px}.breakdown-table td{text-align:right}.breakdown-table td:nth-child(2){text-align:left}.breakdown-table input,.breakdown-table select{width:100%;min-height:32px;padding:0 6px;font-size:10.5px}.breakdown-table th:nth-child(1){width:74px}.breakdown-table th:nth-child(2){width:210px}.breakdown-table th:nth-child(3),.breakdown-table th:nth-child(5){width:67px}.breakdown-table th:nth-child(4){width:82px}.breakdown-table th:nth-child(6){width:92px}.breakdown-table th:nth-child(7){width:150px}.breakdown-table th:nth-child(8),.breakdown-table th:nth-child(9){width:96px}.calculated{display:block;padding:8px 6px;color:var(--muted,#888780);font-size:10px}.contingency-row{background:var(--accent-soft,rgba(186,117,23,.11))}.mini{min-height:30px;border:1px solid var(--hair,#E7E4DA);border-radius:7px;background:transparent;font-size:9.5px;cursor:pointer}.icon{width:30px;height:30px;padding:0;border:1px solid var(--hair,#E7E4DA);border-radius:8px;background:transparent;color:var(--muted,#888780);cursor:pointer}.add-row{min-height:36px;margin:10px 12px 12px;padding:0 13px;border:1px dashed var(--muted,#888780);border-radius:8px;background:transparent;font-weight:700;font-size:10.5px;cursor:pointer}.funding-table{min-width:1120px!important}.funding-table input,.funding-table select,.expense-table input,.expense-table select{width:100%;min-height:32px;padding:0 6px;font-size:10.5px}.funding-table td,.expense-table td{text-align:left;padding:7px 6px}.funding-table td:nth-child(3),.funding-table td:nth-child(4),.expense-table td:nth-child(n+6):nth-child(-n+8){text-align:right}.finance-grid{grid-template-columns:1.4fr .6fr}.cash-timeline{display:grid;gap:12px;margin-top:20px}.cash-period>div{display:flex;justify-content:space-between;gap:12px;font-size:10.5px}.cash-period i{display:block;height:6px;margin-top:5px;border-radius:99px;background:var(--soft,#EFEBE1);overflow:hidden}.cash-period b{display:block;height:100%;border-radius:99px;background:var(--accent,#BA7517)}.finance-note{display:flex;flex-direction:column;align-items:flex-start;justify-content:center}.finance-note p{font-size:11.5px;line-height:1.55;color:var(--muted,#888780)}.finance-note button{padding:8px 0}.expense-card{margin-top:2px}.expense-table{min-width:1500px!important}.expense-table th:nth-child(3){width:250px}.expense-table th:nth-child(4),.expense-table th:nth-child(5){width:180px}.compression-note{padding:7px 10px;border-radius:99px;background:var(--accent-soft,rgba(186,117,23,.11));font-size:9.5px;color:var(--accent,#BA7517);font-weight:700}.table-empty{text-align:center!important;padding:34px!important;color:var(--muted,#888780)}.attach,.receipt label{display:inline-flex;align-items:center;min-height:28px;padding:0 9px;border:1px dashed var(--muted,#888780);border-radius:7px;font-size:9.5px;font-weight:700;cursor:pointer}.attach input,.receipt label input{display:none}.receipt{display:flex;align-items:center;gap:6px;max-width:190px}.receipt button{border:0;background:transparent;color:var(--accent,#BA7517);font-size:9.5px;font-weight:800;cursor:pointer}.receipt span{max-width:74px;overflow:hidden;text-overflow:ellipsis;font-size:9px}.receipt label{min-height:25px;padding:0 6px}.settings-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px}.settings-panel{padding:20px}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:13px;margin-top:20px}.form-grid label:nth-child(5){grid-column:1 / -1}.suffix{display:grid;grid-template-columns:1fr 34px}.suffix input{border-radius:8px 0 0 8px}.suffix b{display:grid;place-items:center;border:1px solid var(--hair,#E7E4DA);border-left:0;border-radius:0 8px 8px 0;background:var(--soft,#EFEBE1);font-size:11px}.driver-result{grid-column:1 / -1;padding:12px 13px;border-radius:10px;background:var(--soft,#EFEBE1)}.driver-result span,.driver-result small{display:block;font-size:9.5px;color:var(--muted,#888780)}.driver-result strong{display:block;margin:5px 0;font-size:16px}.tax-layout{display:grid;grid-template-columns:minmax(420px,1fr) minmax(300px,.75fr);gap:16px;padding:18px}.tax-table{width:100%;border-collapse:collapse}.tax-table input{width:100%}.tax-table td:nth-child(2){width:130px}.tax-table td:last-child{width:42px}.formula-card{padding:16px;border:1px solid var(--hair,#E7E4DA);border-radius:12px;background:var(--soft,#EFEBE1)}.formula-card p{margin:11px 0 0;font-size:10.5px;line-height:1.5;color:var(--muted,#888780)}.formula-card strong{color:var(--ink,#2C2C2A)}.locked{font-size:9px;color:var(--muted,#888780)}.modal-backdrop{position:fixed;inset:0;z-index:1000;display:grid;place-items:center;padding:24px;background:rgba(20,20,19,.58)}.schedule-modal{width:min(720px,100%);max-height:min(760px,90vh);overflow:auto;background:var(--surface,#FFFEF9);border:1px solid var(--hair,#E7E4DA);border-radius:19px 16px 20px 14px;box-shadow:0 24px 80px rgba(0,0,0,.25)}.modal-head{display:flex;justify-content:space-between;gap:18px;padding:20px;border-bottom:1px solid var(--hair,#E7E4DA)}.modal-head>div>span{font-size:10px;font-weight:800;color:var(--accent,#BA7517)}.modal-head h3{font-size:19px}.schedule-summary{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;padding:16px 20px;background:var(--soft,#EFEBE1)}.schedule-summary div{padding:11px;background:var(--surface,#FFFEF9);border:1px solid var(--hair,#E7E4DA);border-radius:9px}.schedule-summary span,.schedule-summary strong{display:block}.schedule-summary span{font-size:9.5px;color:var(--muted,#888780)}.schedule-summary strong{margin-top:6px;font-size:14px}.schedule-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;padding:20px}.modal-foot{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:15px 20px;border-top:1px solid var(--hair,#E7E4DA)}.modal-foot span{font-size:10px;color:var(--muted,#888780)}.notice{margin-bottom:16px;padding:11px 13px;border-radius:9px;font-size:11px}.notice.error{background:rgba(199,68,64,.12);color:#C74440}.loading,.empty{display:grid;justify-items:center;max-width:560px;margin:50px auto;padding:42px;text-align:center;background:var(--surface,#FFFEF9);border:1px solid var(--hair,#E7E4DA);border-radius:17px 14px 18px 13px}.loading span{width:26px;height:26px;border:2px solid var(--hair,#E7E4DA);border-top-color:var(--accent,#BA7517);border-radius:50%;animation:spin .7s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.loading strong,.empty strong{margin-top:15px;font-size:17px}.loading small,.empty p{margin:7px 0 0;color:var(--muted,#888780);font-size:11.5px}.empty .primary{margin-top:18px}.empty.small{margin:20px auto;padding:26px}
      ...
      /* Phase allocation uses readable bars instead of a ring. Each row is a
         real navigation target: hover/focus reveals the largest cost lines,
         and click opens Breakdown filtered to that production phase. */
      .allocation{overflow:visible;contain:none}.allocation-bars{display:grid;gap:5px;margin-top:18px;min-width:0}.allocation-bar-row{position:relative;z-index:1;display:grid;gap:7px;width:100%;min-width:0;padding:10px 8px 11px;border:1px solid transparent;border-radius:11px 9px 12px 8px;background:transparent;color:var(--ink,#2C2C2A);text-align:left;cursor:pointer;transition:transform .16s cubic-bezier(.2,.8,.2,1),background-color .16s ease,border-color .16s ease,box-shadow .16s ease}.allocation-bar-row:hover,.allocation-bar-row:focus-visible{z-index:5;transform:translateX(2px);border-color:color-mix(in srgb,var(--allocation-color,var(--accent,#BA7517)) 32%,var(--hair,#E7E4DA));background:color-mix(in srgb,var(--soft,#EFEBE1) 68%,transparent);box-shadow:0 5px 15px rgba(35,35,34,.07)}.allocation-bar-meta{display:flex;align-items:center;justify-content:space-between;gap:14px;min-width:0}.allocation-bar-name{display:flex;align-items:center;gap:9px;min-width:0}.allocation-bar-name i{display:block;width:9px;height:9px;flex:0 0 9px;border-radius:50%;box-shadow:0 0 0 3px color-mix(in srgb,var(--allocation-color,var(--accent,#BA7517)) 15%,transparent)}.allocation-bar-name strong{min-width:0;overflow:hidden;font-size:10.5px;text-overflow:ellipsis;white-space:nowrap}.allocation-bar-value{display:flex;align-items:baseline;justify-content:flex-end;gap:9px;flex:0 0 auto;font-variant-numeric:tabular-nums}.allocation-bar-value b{font-size:10.5px}.allocation-bar-value small{color:var(--muted,#888780);font-size:9px}.allocation-bar-track{display:block;width:100%;height:9px;overflow:hidden;border-radius:99px;background:var(--soft,#EFEBE1);box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--hair,#E7E4DA) 72%,transparent)}.allocation-bar-track i{display:block;height:100%;min-width:2px;border-radius:inherit;transform-origin:left center;transition:width .35s cubic-bezier(.2,.8,.2,1),filter .18s ease}.allocation-bar-row:hover .allocation-bar-track i,.allocation-bar-row:focus-visible .allocation-bar-track i{filter:saturate(1.12) brightness(1.04)}.allocation-tooltip{position:absolute;right:8px;top:calc(100% - 4px);z-index:10;display:grid;gap:4px;width:min(270px,calc(100% - 16px));padding:11px 12px;border:1px solid color-mix(in srgb,var(--allocation-color,var(--accent,#BA7517)) 38%,var(--hair,#E7E4DA));border-radius:10px 8px 11px 7px;background:color-mix(in srgb,var(--surface,#FFFEF9) 96%,var(--soft,#EFEBE1));box-shadow:0 12px 28px rgba(35,35,34,.16);opacity:0;pointer-events:none;transform:translateY(-3px) scale(.985);transform-origin:top right;transition:opacity .16s ease,transform .18s cubic-bezier(.2,.8,.2,1)}.allocation-bar-row:hover .allocation-tooltip,.allocation-bar-row:focus-visible .allocation-tooltip{opacity:1;transform:none}.allocation-tooltip strong{font-size:10px}.allocation-tooltip span{color:var(--muted,#888780);font-size:8px;font-weight:750;letter-spacing:.45px;text-transform:uppercase}.allocation-tooltip small{color:var(--ink,#2C2C2A);font-size:9px;line-height:1.45}.allocation-tooltip em{color:var(--accent,#BA7517);font-size:8px;font-style:normal;font-weight:750}.view.is-entering .allocation-bar-track i{animation:allocationBarIn .42s .08s cubic-bezier(.2,.8,.2,1) both}@keyframes allocationBarIn{from{transform:scaleX(0);opacity:.35}to{transform:scaleX(1);opacity:1}}.allocation .chart-empty-copy{min-height:180px}.allocation .chart-empty-copy.centered{display:grid;place-content:center;text-align:center}.allocation .chart-empty-copy strong,.allocation .chart-empty-copy span{display:block}.allocation .chart-empty-copy span{margin-top:5px;color:var(--muted,#888780);font-size:9.5px}.allocation .chart-empty-copy strong{font-size:11px}
      @media(max-width:640px){.allocation-bar-meta{gap:8px}.allocation-bar-value{gap:5px}.allocation-bar-value small{font-size:8px}.allocation-tooltip{right:4px;width:calc(100% - 8px)}}
      @media(prefers-reduced-motion:reduce){.allocation-bar-row,.allocation-bar-track i,.allocation-tooltip{transition-duration:.01ms!important}.view.is-entering .allocation-bar-track i{animation:none!important}.allocation-bar-row:hover,.allocation-bar-row:focus-visible{transform:none!important}}
      /* The allocation view is a compact vertical column chart. Pastel fills
         keep the hierarchy calm while the tooltip preserves the audit trail. */
      .allocation-chart{position:relative;display:grid;grid-template-columns:repeat(auto-fit,minmax(92px,1fr));align-items:end;gap:10px;min-height:245px;margin-top:18px;padding:13px 5px 0;border-bottom:1px solid var(--hair,#E7E4DA);isolation:isolate}.allocation-chart:before{content:"";position:absolute;inset:14px 5px 42px;z-index:-1;background:repeating-linear-gradient(to bottom,color-mix(in srgb,var(--hair,#E7E4DA) 72%,transparent) 0 1px,transparent 1px 39px);opacity:.7;pointer-events:none}.allocation-column{position:relative;z-index:1;display:grid;grid-template-rows:18px minmax(132px,1fr) auto;align-items:end;gap:7px;min-width:0;min-height:223px;padding:0 3px;border:1px solid transparent;border-radius:12px 10px 13px 9px;background:transparent;color:var(--ink,#2C2C2A);text-align:center;cursor:pointer;transition:transform .18s cubic-bezier(.2,.8,.2,1),background-color .16s ease,border-color .16s ease,box-shadow .16s ease}.allocation-column:hover,.allocation-column:focus-visible{z-index:8;transform:translateY(-3px);border-color:color-mix(in srgb,var(--allocation-color,#C5BBA3) 42%,var(--hair,#E7E4DA));background:color-mix(in srgb,var(--soft,#EFEBE1) 62%,transparent);box-shadow:0 8px 19px rgba(35,35,34,.08)}.allocation-column-value{display:block;align-self:end;color:var(--ink,#2C2C2A);font-size:10px;font-weight:850;font-variant-numeric:tabular-nums}.allocation-column-track{display:flex;align-items:flex-end;justify-content:center;width:100%;height:100%;min-height:132px;border-radius:10px 10px 4px 4px;background:linear-gradient(to top,color-mix(in srgb,var(--soft,#EFEBE1) 80%,transparent),transparent);overflow:hidden}.allocation-column-track i{display:block;width:clamp(28px,46%,54px);min-height:4px;border-radius:10px 10px 4px 4px;box-shadow:inset 0 0 0 1px rgba(255,255,255,.28),0 4px 12px color-mix(in srgb,var(--allocation-color,#C5BBA3) 25%,transparent);transform-origin:center bottom;transition:height .42s cubic-bezier(.2,.8,.2,1),filter .18s ease}.allocation-column:hover .allocation-column-track i,.allocation-column:focus-visible .allocation-column-track i{filter:saturate(1.08) brightness(1.04)}.allocation-column-label{display:grid;justify-items:center;gap:4px;min-width:0;padding:0 2px 9px}.allocation-column-label i{display:block;width:8px;height:8px;border-radius:50%;box-shadow:0 0 0 3px color-mix(in srgb,var(--allocation-color,#C5BBA3) 16%,transparent)}.allocation-column-label strong{display:block;max-width:100%;overflow:hidden;font-size:9.5px;line-height:1.2;text-overflow:ellipsis;white-space:nowrap}.allocation-column-label small{display:block;max-width:100%;overflow:hidden;color:var(--muted,#888780);font-size:8px;font-variant-numeric:tabular-nums;text-overflow:ellipsis;white-space:nowrap}.allocation-column .allocation-tooltip{position:absolute;right:auto;bottom:calc(100% - 18px);top:auto;left:50%;width:min(230px,calc(100vw - 40px));padding:11px 12px;transform:translate(-50%,8px) scale(.985);transform-origin:center bottom}.allocation-column:hover .allocation-tooltip,.allocation-column:focus-visible .allocation-tooltip{transform:translate(-50%,0) scale(1)}.view.is-entering .allocation-column-track i{animation:allocationColumnIn .45s .08s cubic-bezier(.2,.8,.2,1) both}@keyframes allocationColumnIn{from{transform:scaleY(0);opacity:.35}to{transform:scaleY(1);opacity:1}}.allocation .chart-empty-copy{min-height:200px}
      @media(max-width:640px){.allocation-chart{grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;min-height:225px;padding-inline:2px}.allocation-column{min-height:205px}.allocation-column-track{min-height:116px}.allocation-column-label strong{font-size:9px}.allocation-column .allocation-tooltip{width:min(220px,calc(100vw - 32px))}}
      @media(prefers-reduced-motion:reduce){.allocation-column,.allocation-column-track i,.allocation-column .allocation-tooltip{transition-duration:.01ms!important}.view.is-entering .allocation-column-track i{animation:none!important}.allocation-column:hover,.allocation-column:focus-visible{transform:none!important}}
      /* Editorial reference-chart treatment: solid columns, pill-shaped tops,
         flat baselines and large numbered labels below the data. */
      .allocation-chart{grid-template-columns:repeat(auto-fit,minmax(104px,1fr));gap:14px;min-height:294px;margin-top:20px;padding:0 8px;border-bottom:0}.allocation-chart:before{display:none}.allocation-column{grid-template-rows:minmax(214px,1fr) auto;min-height:274px;gap:10px;padding:0 4px;border-radius:13px 11px 14px 10px}.allocation-column-value{display:none}.allocation-column-track{min-height:214px;border-radius:0;background:transparent;overflow:visible}.allocation-column-track i{width:clamp(38px,54%,64px);min-height:12px;border-radius:999px 999px 0 0;box-shadow:inset 0 0 0 1px rgba(255,255,255,.2),0 8px 16px color-mix(in srgb,var(--allocation-color,#C5BBA3) 22%,transparent)}.allocation-column-label{align-items:start;justify-items:start;gap:4px;padding:0 2px 10px;text-align:left}.allocation-column-label i{display:none}.allocation-column-index{display:block;color:var(--ink,#2C2C2A);font-size:21px;line-height:1;font-variant-numeric:tabular-nums;letter-spacing:-.8px}.allocation-column-label strong{display:block;max-width:100%;font-size:10px;line-height:1.25}.allocation-column-label small{display:block;max-width:100%;font-size:8px;line-height:1.35}.allocation-column:hover .allocation-column-track i,.allocation-column:focus-visible .allocation-column-track i{filter:saturate(1.08) brightness(1.04);box-shadow:inset 0 0 0 1px rgba(255,255,255,.28),0 11px 22px color-mix(in srgb,var(--allocation-color,#C5BBA3) 30%,transparent)}
      @media(max-width:640px){.allocation-chart{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;min-height:264px;padding-inline:3px}.allocation-column{min-height:246px}.allocation-column-track{min-height:184px}.allocation-column-index{font-size:19px}.allocation-column-label strong{font-size:9px}}
      /* Donut treatment with percentage capsules placed around each colored
         segment. The legend remains a second, keyboard-friendly click target. */
      .allocation .donut-wrap{display:grid;grid-template-columns:minmax(190px,.92fr) minmax(190px,1fr);align-items:center;gap:18px;min-width:0;margin-top:18px}.donut-graphic-wrap{display:grid;place-items:center;min-width:0}.donut-graphic{position:relative;width:min(100%,252px);aspect-ratio:1}.donut-svg{display:block;width:100%;height:100%;overflow:visible}.donut-track,.donut-segment-ring{fill:none;stroke-width:24}.donut-track{stroke:var(--soft,#EFEBE1);opacity:.85}.donut-segment{cursor:pointer;outline:none}.donut-segment-ring{stroke-linecap:butt;transition:stroke-width .18s ease,filter .18s ease,opacity .18s ease}.donut-segment:hover .donut-segment-ring,.donut-segment:focus .donut-segment-ring{stroke-width:29;filter:saturate(1.08) drop-shadow(0 5px 8px rgba(35,35,34,.12))}.donut-hole{fill:var(--surface,#FFFEF9);stroke:color-mix(in srgb,var(--hair,#E7E4DA) 82%,transparent);stroke-width:1}.donut-total{fill:var(--ink,#2C2C2A);font-size:13px;font-weight:850;font-variant-numeric:tabular-nums}.donut-total-caption{fill:var(--muted,#888780);font-size:7px;font-weight:800;letter-spacing:1.1px}.donut-segment-label rect{fill-opacity:.95;stroke:rgba(255,255,255,.65);stroke-width:1;transition:transform .18s cubic-bezier(.2,.8,.2,1),filter .18s ease}.donut-segment-label text{fill:var(--ink,#2C2C2A);font-size:8px;font-weight:850;font-variant-numeric:tabular-nums;pointer-events:none}.donut-segment:hover .donut-segment-label rect,.donut-segment:focus .donut-segment-label rect{transform:scale(1.12);filter:drop-shadow(0 3px 5px rgba(35,35,34,.14))}.donut-segment:hover .donut-segment-label text,.donut-segment:focus .donut-segment-label text{font-size:8.5px}.donut-legend{display:grid;gap:5px;min-width:0}.donut-legend-row{display:grid;grid-template-columns:9px minmax(0,1fr) auto;grid-template-rows:auto auto;align-items:center;column-gap:9px;row-gap:2px;width:100%;padding:8px 9px;border:1px solid transparent;border-radius:10px 8px 11px 7px;background:transparent;color:var(--ink,#2C2C2A);text-align:left;cursor:pointer;transition:transform .16s cubic-bezier(.2,.8,.2,1),background-color .16s ease,border-color .16s ease}.donut-legend-row:hover,.donut-legend-row:focus-visible{transform:translateX(3px);border-color:color-mix(in srgb,var(--segment-color,#C5BBA3) 36%,var(--hair,#E7E4DA));background:color-mix(in srgb,var(--soft,#EFEBE1) 68%,transparent)}.donut-legend-row>i{grid-row:1 / span 2;width:9px;height:9px;border-radius:50%;box-shadow:0 0 0 3px color-mix(in srgb,var(--segment-color,#C5BBA3) 16%,transparent)}.donut-legend-row span{min-width:0;overflow:hidden;font-size:10px;font-weight:750;text-overflow:ellipsis;white-space:nowrap}.donut-legend-row strong{font-size:10px;font-variant-numeric:tabular-nums}.donut-legend-row small{grid-column:2 / span 2;color:var(--muted,#888780);font-size:8px;font-variant-numeric:tabular-nums}.donut-empty{display:grid;place-items:center;min-height:220px;border:1px dashed var(--hair,#E7E4DA);border-radius:14px;color:var(--muted,#888780);font-size:10px}.allocation .donut-wrap .chart-empty-copy{min-height:180px}.allocation .donut-wrap .chart-empty-copy strong,.allocation .donut-wrap .chart-empty-copy span{display:block}.allocation .donut-wrap .chart-empty-copy span{margin-top:5px;color:var(--muted,#888780);font-size:9.5px}
      ...
      /* Shadow DOM does not see the document's ordinary dark-mode selectors.
         Resolve the full Budget palette on the host so every inherited
         surface, input, table and modal stays genuinely dark. */
      :host-context(html[data-filmscript-theme="dark"]){--budget-dark-bg:#171716;--budget-dark-surface:#242422;--budget-dark-raised:#2c2c29;--budget-dark-input:#1e1e1c;--budget-dark-ink:#f7f3ec;--budget-dark-muted:#c7c3ba;--budget-dark-line:rgba(247,243,236,.18);--budget-dark-highlight:rgba(255,255,255,.1);--budget-dark-accent:var(--fs-theme-accent,#FFB703);--bg:var(--budget-dark-bg);--surface:var(--budget-dark-surface);--soft:#353530;--chrome:#101010;--ink:var(--budget-dark-ink);--muted:var(--budget-dark-muted);--hair:var(--budget-dark-line);--accent:var(--budget-dark-accent);--accent-soft:color-mix(in srgb,var(--budget-dark-accent) 20%,transparent);--budget-glass:color-mix(in srgb,var(--budget-dark-surface) 88%,transparent);--budget-glass-strong:color-mix(in srgb,var(--budget-dark-raised) 94%,transparent);--budget-glass-edge:var(--budget-dark-line);--budget-glass-shadow:0 18px 46px rgba(0,0,0,.42);color-scheme:dark}
      :host-context(html[data-filmscript-theme="dark"]) .fs-budget{background:var(--budget-dark-bg);color:var(--budget-dark-ink)}
      :host-context(html[data-filmscript-theme="dark"]) .budget-nav{background:linear-gradient(135deg,color-mix(in srgb,var(--budget-dark-raised) 97%,transparent),color-mix(in srgb,var(--budget-dark-surface) 93%,transparent));box-shadow:var(--budget-glass-shadow),inset 0 1px 0 var(--budget-dark-highlight)}
      :host-context(html[data-filmscript-theme="dark"]) .kpi,:host-context(html[data-filmscript-theme="dark"]) .panel,:host-context(html[data-filmscript-theme="dark"]) .table-card,:host-context(html[data-filmscript-theme="dark"]) .account,:host-context(html[data-filmscript-theme="dark"]) .phase-card,:host-context(html[data-filmscript-theme="dark"]) .formula-card,:host-context(html[data-filmscript-theme="dark"]) .settings-panel,:host-context(html[data-filmscript-theme="dark"]) .cash-week,:host-context(html[data-filmscript-theme="dark"]) .cashflow-bridge,:host-context(html[data-filmscript-theme="dark"]) .cash-profile,:host-context(html[data-filmscript-theme="dark"]) .finance-note,:host-context(html[data-filmscript-theme="dark"]) .expense-status-guide{background:linear-gradient(135deg,color-mix(in srgb,var(--budget-dark-raised) 96%,transparent),color-mix(in srgb,var(--budget-dark-surface) 93%,transparent));border-color:var(--budget-dark-line);box-shadow:var(--budget-glass-shadow),inset 0 1px 0 var(--budget-dark-highlight)}
      :host-context(html[data-filmscript-theme="dark"]) .table-title,:host-context(html[data-filmscript-theme="dark"]) .account summary,:host-context(html[data-filmscript-theme="dark"]) .cash-week summary{background:linear-gradient(90deg,color-mix(in srgb,var(--budget-dark-raised) 74%,transparent),transparent)}
      :host-context(html[data-filmscript-theme="dark"]) input,:host-context(html[data-filmscript-theme="dark"]) select,:host-context(html[data-filmscript-theme="dark"]) .formula-note,:host-context(html[data-filmscript-theme="dark"]) .driver-result,:host-context(html[data-filmscript-theme="dark"]) .schedule-summary div,:host-context(html[data-filmscript-theme="dark"]) .expense-line-button{background:var(--budget-dark-input);border-color:var(--budget-dark-line);color:var(--budget-dark-ink);box-shadow:inset 0 1px 0 rgba(255,255,255,.05)}
      :host-context(html[data-filmscript-theme="dark"]) .tabs,:host-context(html[data-filmscript-theme="dark"]) .budget-actions,:host-context(html[data-filmscript-theme="dark"]) .bar,:host-context(html[data-filmscript-theme="dark"]) .allocation-bar-track,:host-context(html[data-filmscript-theme="dark"]) .account-code{background:color-mix(in srgb,var(--budget-dark-input) 92%,transparent);border-color:var(--budget-dark-line)}
      :host-context(html[data-filmscript-theme="dark"]) .tabs button[aria-pressed="true"],:host-context(html[data-filmscript-theme="dark"]) .budget-actions .secondary{background:var(--budget-dark-raised);border-color:var(--budget-dark-line);color:var(--budget-dark-ink);box-shadow:inset 0 1px 0 var(--budget-dark-highlight),0 5px 14px rgba(0,0,0,.2)}
      :host-context(html[data-filmscript-theme="dark"]) tfoot td{background:var(--chrome);color:var(--budget-dark-ink)}
      :host-context(html[data-filmscript-theme="dark"]) .negative{color:#ff8d86!important}:host-context(html[data-filmscript-theme="dark"]) .notice.error{background:rgba(255,110,101,.16);color:#ffaaa4}
      @media(prefers-reduced-transparency:reduce){:host-context(html[data-filmscript-theme="dark"]) .budget-nav,:host-context(html[data-filmscript-theme="dark"]) .kpi,:host-context(html[data-filmscript-theme="dark"]) .panel,:host-context(html[data-filmscript-theme="dark"]) .table-card,:host-context(html[data-filmscript-theme="dark"]) .account,:host-context(html[data-filmscript-theme="dark"]) .phase-card,:host-context(html[data-filmscript-theme="dark"]) .formula-card,:host-context(html[data-filmscript-theme="dark"]) .settings-panel,:host-context(html[data-filmscript-theme="dark"]) .cash-week,:host-context(html[data-filmscript-theme="dark"]) .cashflow-bridge,:host-context(html[data-filmscript-theme="dark"]) .cash-profile,:host-context(html[data-filmscript-theme="dark"]) .finance-note,:host-context(html[data-filmscript-theme="dark"]) .expense-status-guide,:host-context(html[data-filmscript-theme="dark"]) .schedule-modal,:host-context(html[data-filmscript-theme="dark"]) .loading,:host-context(html[data-filmscript-theme="dark"]) .empty{background:var(--budget-dark-surface);backdrop-filter:none;-webkit-backdrop-filter:none}:host-context(html[data-filmscript-theme="dark"]) .budget-nav{background:var(--budget-dark-raised)}}
